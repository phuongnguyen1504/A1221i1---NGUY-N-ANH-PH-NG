package FuramaCaseStudy.service.impl;

import FuramaCaseStudy.model.Employee;
import FuramaCaseStudy.service.EmployeeService;
import FuramaCaseStudy.util.CSVHelper;
...
//    }
    ...
    public void save(Employee employee) {
        if (employee.getId() > 0) {
            ...
            csvHelper.write(employees, ConstantUtil.PATH.EMPLOYEE, false);
        } else {
            employee.setId(employees.size() + 1);
            employees.add(employee);
            List<Employee> tmp= new ArrayList();
            tmp.add(employee);
            csvHelper.write(tmp, ConstantUtil.PATH.EMPLOYEE, true);
        }
    }
    ...
    private void getFromFile() {
        List<String> lines = csvHelper.read(ConstantUtil.PATH.EMPLOYEE);
        for (String line : lines) {
            ...
            Employee employee = new Employee(id, code, fullName, birthday, ConstantUtil.Gender.valueOf(gender), phone,
                    email, address, ConstantUtil.Degree.valueOf(degree), ConstantUtil.Position.valueOf(position), salary);
            employees.add(employee);
        }
    }
}
